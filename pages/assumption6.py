...
st.set_page_config(page_title="Assumption 6: Account_Opening Service Time", layout="wide")

# Load dataset
df = pd.read_csv("../QMS_dataset.csv")

# Title
st.title("Assumption 6: Account_Opening Has Longest Service Time")
...
# Highlight Account_Opening
account_opening_avg = service_time_avg[service_time_avg["SERVICE_TYPE"] == "Account_Opening"]["SERVICE_TIME_MIN"].iloc[0] if "Account_Opening" in service_time_avg["SERVICE_TYPE"].values else 0

# Create bar chart
fig = px.bar(service_time_avg, x="SERVICE_TYPE", y="SERVICE_TIME_MIN", title="Average Service Time by Service Type")
fig.update_traces(marker_color=["#FF4B4B" if x == "Account_Opening" else "#1F77B4" for x in service_time_avg["SERVICE_TYPE"]])
st.plotly_chart(fig, use_container_width=True)

# Display metric
st.metric("Account_Opening Avg Service Time", f"{account_opening_avg:.2f} min")